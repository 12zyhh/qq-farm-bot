  ...
  <div class="app-container" :class="themeClass">
    ...
    <div v-if="showSplash" class="splash-screen">
      <div class="splash-content">
        ...
        <div class="splash-warnings">
          ...
          <p class="splash-warning">感谢linguo2625469、QianChenJun的开源贡献</p>
        </div>
        <div class="splash-divider"></div>
        <a class="splash-link" @click="openGithub">
          🔗 GitHub: github.com/12zyhh/qq-farm-bot
        </a>
        <div class="splash-divider"></div>
    ...
    <div class="titlebar">
      <span class="titlebar-title">
        QQ经典农场助手 - 开源免费项目 |
        <a class="titlebar-link" @click="openGithub">github.com/12zyhh/qq-farm-bot</a>
      </span>
      <div class="titlebar-buttons">
        <button class="titlebar-btn theme-toggle" @click="toggleTheme" title="切换主题">
          {{ themeIcon }}
        </button>
      ...
      <div class="content">
        ...
        <div class="watermark">
          <a class="watermark-link" @click="openGithub">
            开源免费 | github.com/12zyhh/qq-farm-bot | 付费购买请退款
          </a>
        </div>
      </div>
    </div>
...
function openGithub() {
  window.electronAPI?.invoke('shell:openExternal', 'https://github.com/12zyhh/qq-farm-bot')
}
...
function toggleTheme() {
  ...
  applyTheme(newTheme)
  window.electronAPI?.invoke('bot:save-config', { theme: newTheme })
}

function applyTheme(themeName: 'dark' | 'light') {
  const darkLink = document.getElementById('theme-dark') as HTMLLinkElement
  const lightLink = document.getElementById('theme-light') as HTMLLinkElement
  const elementDarkLink = document.getElementById('element-dark') as HTMLLinkElement

  if (themeName === 'dark') {
    if (darkLink) darkLink.disabled = false
    if (lightLink) lightLink.disabled = true
    if (elementDarkLink) elementDarkLink.disabled = false
  } else {
    if (darkLink) darkLink.disabled = true
    if (lightLink) lightLink.disabled = false
    if (elementDarkLink) elementDarkLink.disabled = true
  }
}

async function loadTheme() {
  const config = await window.electronAPI?.invoke('bot:get-config')
...
provide('setTheme', (themeName: 'dark' | 'light') => {
...
.app-container.light .titlebar-btn:hover {
  background: rgba(0, 0, 0, 0.05);
  color: var(--color-text);
}

.app-container:not(.light) .titlebar-btn:hover {
...
.app-container.light .splash-screen {
  background: rgba(255, 255, 255, 0.95);
}

.app-container.light .splash-divider {
  background: rgba(0, 0, 0, 0.1);
}

.app-container.light .watermark-link {
  background: rgba(0, 0, 0, 0.6);
  color: rgba(255, 255, 255, 0.7);
  border-color: rgba(255, 255, 255, 0.1);
}

.app-container.light .watermark-link:hover {
  background: rgba(0, 0, 0, 0.8);
  color: rgba(255, 255, 255, 0.9);
}
</style>
